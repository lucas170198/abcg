...
float Car::mapPositionToXCord(Position pos){
  switch (pos){
    case Position::Midle:
      return 0;
      break;
...
void Car::initializeGL(GLuint program) {
  ...
  m_translation = glm::vec2{0, initialyPos};
  ...
  std::array<glm::vec2, 24> positions{
      ...
      // whell up left
      glm::vec2{-2.7f,  +3.0f}, glm::vec2{-3.7f, +3.0f},
      glm::vec2{-3.7f,  +2.0f}, glm::vec2{-2.7f, +2.0f},
...
void Car::update(const GameData &gameData){
   // Turn
    m_translation.x = mapPositionToXCord(gameData.m_mainCarPos);
}
